...
const EthicsGate = ({ onAgree }) => {
  ...
  // 컨텐츠가 짧아서 애초에 스크롤이 안 생기는 경우를 대비한 체크
  useEffect(() => {
    if (contentRef.current) {
    ...
    <div className="ethics-gate-container">
      <div className="ethics-gate-card">
        <div className="ethics-header">
          <h2>초등윤리규정</h2>
          <p>본 활동을 시작하기 전에 아래의 핵심 가이드를 반드시 읽어주세요.</p>
        </div>
        
        <div 
          className="ethics-content-scroll" 
          ref={contentRef} 
        ...
        >
          {guides.map((guide, index) => (
            <div key={index} className="guide-item">
              <div className="guide-tags">
                {guide.tags.map((tag, tIndex) => (
                  <span key={tIndex} className="guide-tag" style={{backgroundColor: guide.color}}>{tag}</span>
                ))}
              </div>
          ...
          {!isScrolledToBottom && (
            <div className="scroll-hint">
              <span className="material-symbols-outlined bounce">keyboard_arrow_down</span>
              <p>내용을 끝까지 읽어주세요</p>
            </div>
          )}
        </div>
        ...
        <div className="ethics-footer">
          <button 
            className={`btn-agree ${isScrolledToBottom ? 'active' : 'disabled'}`}
            onClick={() => isScrolledToBottom && onAgree()}
